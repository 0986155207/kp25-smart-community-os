...
export default function ThanhCongBanner({ phanAnhId, shortCode }: Props) {
  ...
  const isNew = searchParams.get('moi') === '1'

  const [visible, setVisible] = useState(false)
  const [copied,  setCopied]  = useState(false)

  // Chỉ hiện sau khi client mount + có ?moi=1
  useEffect(() => {
  ...
  return (
    <div className="mb-6 bg-gradient-to-r from-emerald-50 to-green-50 border-2 border-emerald-200 rounded-2xl p-5 relative">
      <button
        ...
        className="absolute top-3 right-3 w-7 h-7 rounded-full bg-white/80 flex items-center
                   justify-center text-slate-400 hover:text-slate-600 hover:bg-white transition-all"
      >
        <X size={14} />
      </button>
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0 pr-6">
          <h3 className="font-bold text-emerald-800 text-base">Gửi phản ánh thành công!</h3>
          ...
          <div className="mt-3 flex items-start gap-2 p-3 bg-white/70 rounded-xl border border-emerald-100">
            ...
            <p className="text-xs text-slate-600 leading-relaxed">
              <strong>Lưu trang này</strong> để theo dõi tiến độ, hoặc{' '}
