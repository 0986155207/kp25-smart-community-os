...
async function registerServiceWorker(): Promise<ServiceWorkerRegistration | null> {
  ...
  try {
    ...
    // Inject Firebase config vào SW (SW không đọc được process.env)
    const sw = reg.active ?? reg.installing ?? reg.waiting
    sw?.postMessage({ type: 'FIREBASE_CONFIG', config: FIREBASE_CONFIG })

    return reg
  } catch (err) {
    console.error('[FCM Portal] Đăng ký SW thất bại:', err)
    return null
  }
}
...
export async function layFCMToken(): Promise<{
  ...
  if (!daCoFirebaseConfig()) {
    return { token: null, error: 'Chưa cấu hình Firebase' }
  }
  ...
  if (permission !== 'granted') {
    return { token: null, denied: true, error: 'Bạn chưa cho phép nhận thông báo' }
  }
  ...
  try {
    ...
    console.error('[FCM Portal] Lỗi lấy token:', err)
